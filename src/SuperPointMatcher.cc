...
namespace ORB_SLAM3
{
    ...
    //     matchDescriptorsBF(mapPointsDescriptorsValid, F.mDescriptors, matches);
    ...
    int SuperPointMatcher::SearchByProjection(KeyFrame* pKF, Sophus::Sim3f &Scw, const vector<MapPoint*> &vpPoints,
    ...
    int SuperPointMatcher::SearchByProjection(KeyFrame* pKF, Sophus::Sim3<float> &Scw, const std::vector<MapPoint*> &vpPoints, const std::vector<KeyFrame*> &vpPointsKFs,
    ...
    int SuperPointMatcher::Fuse(KeyFrame *pKF, Sophus::Sim3f &Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
    {
        // Get Calibration Parameters for later projection
        const float &fx = pKF->fx;
        const float &fy = pKF->fy;
    ...
    int SuperPointMatcher::SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, std::vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12, const float th)
    {
    ...
    void SuperPointMatcher::ComputeThreeMaxima(vector<int>* histo, const int L, int &ind1, int &ind2, int &ind3)
    {
        int max1=0;
        int max2=0;
        int max3=0;
    ...
    void SuperPointMatcher::matchDescriptorsBF(
        ...
        // 2. Создание BFMatcher
        cv::BFMatcher matcher(normType, crossCheck);

        // 3. Поиск соответствий
        if (crossCheck) {
            // Кросс-проверка (только взаимно лучшие соответствия)
